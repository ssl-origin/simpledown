...
class acp_settings_controller
{
    ...
    protected function upload_file()
    {
        ...
        // 1. Nova miniatura enviada (prioridade máxima)
        $thumb_file = $this->request->file('thumb_upload');
        if (!empty($thumb_file['name'])) {
            $allowed = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
            $finfo = finfo_open(FILEINFO_MIME_TYPE);
            $mime = finfo_file($finfo, $thumb_file['tmp_name']);
            finfo_close($finfo);

            if (!in_array($mime, $allowed)) {
                @unlink($dest);
                $this->template->assign_vars(['ERROR_MESSAGE' => $this->language->lang('ACP_SIMPLEDOWN_INVALID_IMAGE_FORMAT')]);
                return;
            }
            ...
            if (move_uploaded_file($thumb_file['tmp_name'], $thumb_dest)) {
                ...
                $this->log->add('admin', $this->user->data['user_id'], $this->user->ip, 'LOG_SIMPLEDOWN_THUMB_ADDED', false, [$thumb_filename]);
            } else {
                $this->template->assign_vars(['WARNING_MESSAGE' => $this->language->lang('ACP_SIMPLEDOWN_THUMB_UPLOAD_FAILED')]);
            }
        }
        // 2. Senão, usa miniatura existente selecionada
        else {
        ...
        $sql_data = [
            ...
            // === ADIÇÃO: DATA DE UPLOAD AUTOMÁTICA PARA NOVOS ARQUIVOS ===
            'added_time'                => time(),
        ];
        ...
        // CRIAÇÃO AUTOMÁTICA DO TÓPICO DE ANÚNCIO (mantido exatamente como estava)

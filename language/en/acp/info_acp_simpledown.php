...
$lang = array_merge($lang, [
    ...
    'ACP_SIMPLEDOWN_ALLOW_USER_LAYOUT_CHOICE_EXPLAIN' => 'When enabled, visitors can toggle between grid (cards) and list views on the public downloads page. The choice is saved in a cookie.',
    ...
    'ACP_SIMPLEDOWN_PRIVATE_CATEGORIES_EXPLAIN' => 'Select categories where files will require login for download. Leave empty so no category is private by default (individual control still works).',
    ...
    'ACP_SIMPLEDOWN_REPLACE_FILE_HELP'       => 'The old file will be permanently deleted. The new one will be saved with a sanitized name (it may be automatically renamed if there is a conflict).',
    ...
    'ACP_SIMPLEDOWN_ANNOUNCE_EXPLAIN'        => 'When enabled, a topic will be automatically created in the configured forum when a new file is added.',
    ...
    'ACP_SIMPLEDOWN_ANNOUNCE_MESSAGE_EXPLAIN' => 'Available placeholders:<br><strong>{NAME}</strong> – File name<br><strong>{VERSION}</strong> – Version<br><strong>{DESC_SHORT}</strong> – Short description<br><strong>{DESC_FORMATTED}</strong> – Full description with BBCode<br><strong>{URL_DETAILS}</strong> – Link to details page<br><strong>{URL_DOWNLOAD}</strong> – Direct download link<br><br><em>Full example already filled by default.</em>',
    ...
    'LOGS_CLEARED_OLD_SUCCESS'              => 'Old logs have been deleted successfully.',
    ...
    'ACP_SIMPLEDOWN_ASSOCIATED_FILES_EXPLAIN' => 'Files in the <code>/files/</code> folder that are linked to an active record in the database (in use).',
    ...
    'ACP_SIMPLEDOWN_ORPHAN_FILES_EXPLAIN'   => 'Physical files in the <code>/files/</code> folder that are not associated with any record in the database. These can be safely deleted.',
    ...
    'ACP_SIMPLEDOWN_TOOLS_EXPLAIN'          => 'Write any content here you wish to display in the Tools tab.<br />Supports basic HTML (links, images, lists, bold, etc.).',
    ...
    'ACP_SIMPLEDOWN_FILE_DUPLICATE_DETECTED'=> 'This file has been uploaded before as "<strong>%1$s</strong>" (server name: <code>%2$s</code>).<br />Use the <strong>Files</strong> tab to edit it or upload a different version.',
